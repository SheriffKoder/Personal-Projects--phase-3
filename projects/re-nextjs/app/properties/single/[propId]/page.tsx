...
const page = () => {
    ...
    async function fetchNews () {
        ...
        //get this page's property
        const response = await fetch(`/api/properties/single/${current_url}`);
        // const response = await fetch("/api/properties/single/66389a4c71c70b5b87d9f211");
        ...
        const resProperty = await response.json();
        // setIsLoading(false);
        ...
        if (resProperty) {
            //set a title for the property manually
            //check for the thisProperty key because nextJs gives an error .thisProperty does not exist
            if (resProperty.hasOwnProperty("thisProperty")) {

                let property_title_temp = `${resProperty.thisProperty.property_type} for ${resProperty.thisProperty.property_listing_type} in <${resProperty.thisProperty.property_country} ${resProperty.thisProperty.property_city} ${resProperty.thisProperty.property_district} ${resProperty.thisProperty.property_area}sqm ${resProperty.thisProperty.property_beds} bedrooms / ${resProperty.thisProperty.property_baths} bathrooms`;

                setPropertyTitle(property_title_temp);
                //Part 11 - filter out empty strings in the images array in the property (i.e not used image slots)
                resProperty.thisProperty.property_images = resProperty.thisProperty.property_images
                .filter((image: string) => {
                    if (image !== "") {
                        return image;
                    }
    ...
    if (pageProperty !== null && pageProperty.thisProperty.property_userId ) {
        return (
            ...
            <div className="mx-auto max-w-[1230px]">
                ...
                &#62;
                <Link className="" href="/properties/all">Properties</Link>
                &#62;
                    ...
                    {pageProperty.thisProperty ? (
                        <>
                            ...
                            <div className="flex flex-col gap-6 w-[100%] md2:flex-1">
                                ...
                                <PropertyImage pageProperty={pageProperty} property_title={propertyTitle}/>
                                        ...
                                        <div className="h-full flex flex-col gap-4 mt-2 capitalize text-center w-full items-center">
                                            ...
                                            <div className="w-full mx-auto lg:w-auto lg:mx-0 mt-auto md2:mt-0 mb-2">
                                                <div className="flex flex-row flex-wrap gap-4 md2:gap-8 md2:flex-col mt-auto">
                                                    {pageProperty.recProperties.map((property, index) => (
                                                        
                                                        // md:w-[calc(100%*(1/3)-11px)] 
                                                        <div key={index} className="w-[calc(100%*(1/2)-8px)] md2:w-full">
