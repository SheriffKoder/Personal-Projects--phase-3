// import React from 'react'

import Link from "next/link";
import Image from "next/image";


const Hero = () => {
  return (
    
    <div className="main__hero flex flex-col justify-center items-center 
    md:items-start md:px-[5rem] before:opacity-[0.3] dark:before:opacity-[0.05]">

        <Image src="/images/furniture.avif" alt="Furniture" fill={true} className="hero-brush-mask w-full">
        </Image>
        
        <header className="main__hero__text-container 
        flex flex-col justify-center items-center md:items-start
        p-7 m-auto">

            <h1 className="text-[min(calc(1rem+2vw),(2.5rem))] dark:text-[#7d7c7c] text-[#464646] font-semibold uppercase">
                Welcome to
            </h1>

            <h1 className="text-[min(calc(2rem+2vw),(4.5rem))] text-white font-semibold uppercase">
                Richard's <span className="text-[min(calc(2rem+2vw),(4.5rem))] font-light text-center">Real Estate</span>
            </h1>

            <h1 className="text-[min(calc(1rem+0.5vw),(1rem))] dark:text-[#7d7c7c] text-[#464646] font-normal">
                Your preferred real estate hub
            </h1>

            <Link href="/properties" className=" text-[min(calc(1.0rem+0.2vw),(1.2rem))]
            mt-3 bg-theme-text-brighter dark:bg-theme-text-dark text-white 
            rounded-full py-1.5 px-3 w-fit 
            opacity-80 hover:opacity-90">
                    View all properties
            </Link>


        

        </header>




    </div>
  )
}

export default Hero;